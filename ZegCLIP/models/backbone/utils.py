...
from models.backbone.loralib import LoRAMultiheadAttention
...
class LoRATransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, drop_path_rate=0.):
        ...
        self.layers = layers
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, layers)]  # stochastic depth decay rule
        self.resblocks = nn.Sequential(*[LoRAResidualAttentionBlock(width, heads, attn_mask, dpr[i]) for i in range(layers)])
